export class Bubblewel {
  ...
  draw(ctx) {
    ...
    // Adjusted gradient colors for a water blue effect
    gradient.addColorStop(0, `rgba(173, 216, 230, ${this.alpha * 1.3})`); // Light water blue
    gradient.addColorStop(0.5, `rgba(135, 206, 235, ${this.alpha})`); // Sky blue
    gradient.addColorStop(1, `rgba(70, 130, 180, ${this.alpha * 0.5})`); // Steel blue

    ctx.fillStyle = gradient;

    // Add a subtle shadow for depth
    ctx.shadowColor = 'rgba(135, 206, 235, 0.5)'; // Sky blue shadow
    ctx.shadowBlur = 15;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 0;
    ...
    ctx.fillStyle = `rgba(255, 255, 255, ${this.alpha * 0.7})`; // White highlight
    ctx.fill();
    ctx.closePath();
  }
